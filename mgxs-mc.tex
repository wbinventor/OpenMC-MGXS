...
It should be emphasized that the notation above uses the angular flux rather than the scalar flux varying in energy and space for energy collapsing and spatial averaging, since the angular flux is used to collapse MGXS in OpenMC. Although this paper assumes the entire angular domain is integrated over for MGXS generation, the implementation in OpenMC are generalizable and also permit the tabulation of angular-dependent MGXS, which will be a topic explored in future publication(s).
