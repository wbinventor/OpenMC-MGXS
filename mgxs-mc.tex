...
\noindent These estimators are used for reaction types which are only dependent on the energyin of a neutron.
...
\todo[inline]{PR: We should really define $\Sigma_{s\ell}$ here. Otherwise it's not clear what $\Sigma_{s1}$ is. The definition should include $d\mathbf{\Omega'}$ as a variable of integration.}
\begin{equation}
\label{eqn:sigs1}
...
\noindent The spatially-homogenized and energy condensed transport-corrected total cross section is computed by summing over all energyin groups:
...
%\begin{equation}
%\langle \sigma_{s,g'\rightarrow g} \phi \rangle = \int_{r \in V} dr \int_{4\pi} d\Omega' \int_{E_{g'}}^{E_{g'-1}} dE' \int_{4\pi} d\Omega \int_{E_g}^{E_{g-1}} dE \; \sigma_{s} (r, E' \rightarrow E, \Omega' \cdot \Omega) \psi(r, E', \Omega')
%\end{equation}
%
%\begin{equation}
%\langle \sigma_{s,0,g'} \phi \rangle = \int_{r \in V} dr \int_{4\pi} d\Omega' \int_{E_{g'}}^{E_{g'-1}} dE' \int_{4\pi} d\Omega \int_{0}^{\infty} dE \; \sigma_s (r, E' \rightarrow E, \Omega' \cdot \Omega) \psi(r, E', \Omega')
%\end{equation}
...
\begin{table}[h!]
  ...
  \cline{3-4}
  & & $\langle \psi \rangle_{k,g}$ & space, energyin \\

  \midrule

  \multirow{2}{*}{\bf Total} & \multirow{2}{*}{$\hat{\Sigma}_{t,k,g}$} & $\langle \Sigma_{t}, \psi \rangle_{k,g}$ & space, energyin \\
  \cline{3-4}
  & & $\langle \psi \rangle_{k,g}$ & space, energyin \\

  \midrule

  \multirow{3}{*}{\parbox{1.5cm}{\bf Radiative Capture}} & \multirow{3}{*}{$\hat{\Sigma}_{\gamma,k,g}$} & $\langle \Sigma_{a}, \psi \rangle_{k,g}$ & space, energyin \\
  \cline{3-4}
  & & $\langle \Sigma_{f}, \psi \rangle_{k,g}$ & space, energyin \\
  \cline{3-4}
  & & $\langle \psi \rangle_{k,g}$ & space, energyin \\
  ...
  \multirow{2}{*}{\parbox{1.5cm}{\bf Scattering Matrix}} & \multirow{2}{*}{$\hat{\Sigma}_{s\ell,k,g'\rightarrow g}$} & $\langle \Sigma_{s\ell}, \psi \rangle_{k,g'\rightarrow g}$ & space, energyin, energyout \\
  \cline{3-4}
  & & $\langle \psi \rangle_{k,g}$ & space, energyin \\

  \midrule

  \multirow{3}{*}{\parbox{1.5cm}{\bf Scattering Matrix (consistent)}} & \multirow{3}{*}{$\hat{\Sigma}_{s\ell,k,g'\rightarrow g}$} & $\langle \Sigma_{s\ell}, \psi \rangle_{k,g}$ & space, energyin \\
  \cline{3-4}
  & & $\langle \psi \rangle_{k,g}$ & space, energyin \\
  \cline{3-4}
  & & $\langle \Sigma_{s\ell}, \psi \rangle_{k,g'\rightarrow g}$ & space, energyin, energyout \\
  \midrule
  ...
  \parbox{1.5cm}{\parbox{1.2cm}{\bf Fission Spectrum}} & $\hat{\chi}_{k,g}$ & $\langle \nu\Sigma_{f}, \psi \rangle_{k,g'\rightarrow g}$ & space, energyin, energyout \\

  \midrule

  \parbox{1.5cm}{\parbox{1.2cm}{\bf Inverse Velocity}} & \multirow{2}{*}{$\hat{\upsilon}_{k,g}^{-1}$} & $\langle \upsilon^{-1}, \psi \rangle_{k,g}$ & space, energyin \\
  \cline{3-4}
  & & $\langle \psi \rangle_{k,g}$ & space, energyin \\
