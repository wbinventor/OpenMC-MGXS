...
\begin{table}[h!]
  ...
  \parbox{1.5cm}{\parbox{1.2cm}{\bf Inverse Velocity}} & \multirow{2}{*}{$\hat{\upsilon}_{k,g}^{-1}$} & $\langle \upsilon^{-1}, \psi \rangle_{k,g}$ & \texttt{total} & spatial domain\ref{foot:domain}, \texttt{energy} \\
  \cline{3-5}
  & & $\langle \psi \rangle_{k,g}$ & \texttt{flux} & spatial domain\ref{foot:domain}, \texttt{energy} \\

  \midrule
  \multicolumn{5}{c}{\bf Prompt Neutron Specific Constants} \\
  \midrule

  \multirow{2}{*}{\parbox{1.5cm}{\bf Fission \hspace{1cm} Production}} & \multirow{2}{*}{$\nu\hat{\Sigma}_{f,k,g}^{p}$} & $\langle \nu\Sigma_{f}^{p}, \psi \rangle_{k,g}$ & \texttt{prompt-nu-fission} & spatial domain\ref{foot:domain}, \texttt{energy} \\
  \cline{3-5}
  & & $\langle \psi \rangle_{k,g}$ & \texttt{flux} & spatial domain\ref{foot:domain}, \texttt{energy} \\

  \midrule

  \parbox{1.5cm}{\parbox{1.2cm}{\bf Fission Spectrum}} & $\hat{\chi}_{k,g}^{p}$ & $\langle \nu\Sigma_{f}^{p}, \psi \rangle_{k,g'\rightarrow g}$ & \texttt{prompt-nu-fission} & spatial domain\ref{foot:domain}, \texttt{energy}, \texttt{energyout} \\

  \midrule
  \multicolumn{5}{c}{\bf Delayed Neutron Specific Constants} \\
  \midrule

  \multirow{2}{*}{\parbox{1.5cm}{\bf Fission \hspace{1cm} Production}} & \multirow{2}{*}{$\nu\hat{\Sigma}_{f,k,g}^{d}$} & $\langle \nu\Sigma_{f}^{d}, \psi \rangle_{k,g}$ & \texttt{delayed-nu-fission} & spatial domain\ref{foot:domain}, \texttt{energy}, \texttt{delayedgroup} \\
  \cline{3-5}
  & & $\langle \psi \rangle_{k,g}$ & \texttt{flux} & spatial domain\ref{foot:domain}, \texttt{energy} \\

  \midrule

  \parbox{1.5cm}{\parbox{1.2cm}{\bf Fission Spectrum}} & $\hat{\chi}_{k,g}^{d}$ & $\langle \nu\Sigma_{f}^{d}, \psi \rangle_{k,g'\rightarrow g}$ & \texttt{delayed-nu-fission} & spatial domain\ref{foot:domain}, \texttt{energy}, \texttt{energyout}, \texttt{delayedgroup} \\

  \midrule

  \multirow{2}{*}{\parbox{1.5cm}{\bf Precursor \hspace{1cm} Decay Rate}} & \multirow{2}{*}{$\hat{\lambda}_{k,g}^{d}$} & $\langle \nu\Sigma_{f}^{d}, \psi \rangle_{k,g}$ & \texttt{delayed-nu-fission} & spatial domain\ref{foot:domain}, \texttt{energy}, \texttt{delayedgroup} \\
  \cline{3-5}
  & & $\Big\langle \frac{1}{\lambda^d} \nu\Sigma_{f}^{d}, \psi \Big\rangle_{k,g}$ & \texttt{delayed-nu-fission} & spatial domain\ref{foot:domain}, \texttt{energy}, \texttt{delayedgroup} \\
