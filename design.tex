...
The \texttt{openmc.mgxs} module was developed with general design principles to generate MGXS for any multigroup neutron transport code. Although the module does not explicitly support any multigroup codes, it can export MGXS data to a variety of data storage formats, including \textcolor{red}{c}omma-separated values (CSV) and HDF5. The exported MGXS files may be easily transformed into the database or input files required by a particular multigroup code. Additionally, a cross section library can be exported to the format used by OpenMC's multigroup transport mode.
